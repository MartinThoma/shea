...
def _iter_entries(path: str, *, show_all: bool) -> list[os.DirEntry]:
    ...
    try:
        ...
        print(f"pyl: permission denied: {path}", file=sys.stderr)
        return []
...
def print_listing(path: str, *, show_all: bool = False) -> None:
    ...
    if Path(path).is_dir():
        ...
        print(f"pyl: no such file or directory: {path}", file=sys.stderr)
...
def build_parser() -> argparse.ArgumentParser:
    """Create and return the argument parser for the pyl CLI."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.depth is not None and args.depth < 0:
        print("pyl: depth must be >= 0", file=sys.stderr)
        return 2
